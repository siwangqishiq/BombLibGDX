...
public class GameScreen extends BaseScreen {
	...
	public void show() {
		...
		plane = new Plane(engine.getHeight(), engine.getStateManager()
				.getCurrentLevel());
		...
		for (int i = 0; i < 2; ++i) {
			Cloud c = new Cloud(engine.getWidth(), engine.getHeight(),
					groundLine.y);
			clouds.add(c);
		}
	...
	public void createBuildings() {
		...
		case 1:
			numBuildings = maxStoreys = 15;
			break;
		case 2:
			numBuildings = maxStoreys = 17;
			break;
		case 3:
			numBuildings = maxStoreys = 19;
			break;
		case 4:
			numBuildings = 21;
			maxStoreys = 20;
			break;
		case 5:
			numBuildings = maxStoreys = 23;
			maxStoreys = 20;
			break;
		default:
			numBuildings = maxStoreys = 25;
			maxStoreys = 20;
			break;
		}
		...
		for (int i = 0; i < numBuildings; ++i) {
			Building b = new Building(numBuildings, i, engine.getWidth(),
					groundLine.y + groundLine.height, maxStoreys);
			buildings.add(b);
		}
	}
	...
	private void updatePlane(final float dTime) {
		if (!plane.isParked()) {
			plane.move(dTime, engine.getWidth(), groundLine.y
					+ groundLine.height);
	...
	private void updateState() {
		// if we've destroyed all the buildings and the plane isn't already
		// parking, set it to park
		if (buildings.size == 0) {
			plane.setToPark();
		}
	...
	private void draw(float dTime) {
		...
		shapeRender.filledRect(0, 0, engine.getWidth(), engine.getHeight());
		for (Building b : buildings) {
			shapeRender.setColor(b.getColor());
			shapeRender.filledRect(b.x, b.y, b.width, b.height);
		}
		shapeRender.setColor(groundColor);
		shapeRender.filledRect(groundLine.x, groundLine.y, groundLine.width,
				groundLine.height);
		shapeRender.end();
		...
		font.draw(batch, "Score = " + engine.getPlayer().getLevelScore(), 0,
				groundLine.y);
		batch.end();
